...
'''    
def getbpseq(ct_dict_all,seq,seq_names):
    print(len(ct_dict_all))
    print(len(seq))
    print(len(seq_names))
    for i in range(len(seq)):
        list1 = [j + 1 for j in range(len(seq[i]))]
        list2 = []
        for j in range(len(seq[i])):
            list2.append(ct_dict_all[i][j+1])
        list3 = []
        for j in seq[i]:
            list3.append(j)
        download = '/hy-tmp/TransUFold/2/' + seq_names[i]
...
def getcsv(seq_names,seq_lens_list,run_time,accuracy,recall,precision,f1_score):
    ...
    df.to_csv("test_512.csv",index=False)
    return
...
def main():
    ...
    #加载模型
    MODEL_SAVED = 'models/TransUFold_512.pt'
    ...
    test_set = Dataset_test(test_data)
